#!/bin/bash
# Mastermind Test - Codex (DEBUG VERSION)
...
echo "=== Codex Mastermind Test (DEBUG) ==="
echo ""
...
# Parse Codex JSONL response with debug output
parse_codex_response_debug() {
    python3 -c "
import sys, json, re
...
print(f'Lines: {len(raw.split(chr(10)))}', file=sys.stderr)
...
for line in raw.split('\n'):
    ...
    try:
        ...
        if obj.get('type') == 'item.completed':
            ...
            print(f'Found item.completed: type={item_type}', file=sys.stderr)

            if item_type == 'agent_message':
                text = item.get('text', '')
                print(f'Agent message text: {repr(text[:200])}...', file=sys.stderr)

                # Try direct parse
                try:
                    data = json.loads(text.strip())
                    print(f'Direct parse succeeded: {data}', file=sys.stderr)
        ...
        print(f'Line parse error: {e}', file=sys.stderr)
        continue
...
# Call Codex with retry logic (debug version)
call_codex_debug() {
    local prompt="$1"
    local attempt=1
    ...
    while [ $attempt -le $MAX_RETRIES ]; do
        ...
        echo "Calling Codex with ${TIMEOUT_SECONDS}s timeout..."
        echo "Model: $MODEL"
        response=$(timeout $TIMEOUT_SECONDS codex exec --json -m "$MODEL" "$prompt" 2>&1)
        exit_code=$?
        ...
        # Parse response
        echo ""
        parse_output=$(echo "$response" | parse_codex_response_debug)
        echo "$parse_output"
...
Make your next guess. Respond ONLY with JSON: {\"guess\": [a,b,c,d]}"
        ...
        # Get guess from Codex (use tee to show output AND capture it)
        guess_result=$(call_codex_debug "$prompt" | tee /dev/stderr)
